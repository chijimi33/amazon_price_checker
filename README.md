...
同シート右側の自動計算ビューでは、RTX 4060のPassMark G3Dを100とした相対性能、登録64構成内のG3D順位、性能帯、PassMark G3Dをベンチマーク掲載TDPで割った参考効率を表示します。基準GPUは `GPUChips!AA3`、性能帯のしきい値は `Lists!AK4:AL9` で人が変更でき、各行の表示値は数式で再計算されます。これらはPassMarkを読みやすくした比較補助であり、実ゲームFPS、レイトレーシング、アップスケーリング、フレーム生成、個別ボードの電力制限を表す値ではありません。
